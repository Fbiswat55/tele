...
id = '5042903101'
token = '6287815091:AAETlA8hB8wcsY4LrjzaxxYADL3gW32eOw4'
...
def phone(message):
 ...
 if kl == None:
  if glob.glob(f'H/H{message.from_user.id}.txt'):
   ...
   if str(message.from_user.id)== '5042903101':
 ...
 - اضغط على الزر ادناه لكي اتحقق ذلك ...''', reply_markup = keyboard)
   ...
   bot.reply_to(message,'- تم اعطائك محاولتين لزيادة المحاولات راسل المطور ')
     ...
     if kl == None:
      if glob.glob(f'H/H{call.from_user.id}.txt'):
       if int(de(open(f'H/H{call.from_user.id}.txt','r').read())) >= 1:
        u = open('pay.txt').read()
        if str(call.message.chat.id) in u or str(call.message.chat.id) == '5042903101':
...
عبدالعزيز (عبد)
==============================
للمساعدة يرجى التواصل مع المطور : @m3_mv
استمتعوا (؛''')
